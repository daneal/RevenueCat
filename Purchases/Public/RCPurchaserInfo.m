...
- (NSDictionary<NSString *, NSString *> *)nonSubscriptionPurchasesByPurchaseDate
{
    NSMutableDictionary<NSString *, NSString *> *parsedPurchases = [NSMutableDictionary new];
    
    NSDictionary *subscriberData = self.originalData[@"subscriber"];
    if (subscriberData[@"non_subscriptions"] == nil) {
        return parsedPurchases;
    }
    
    NSDictionary *nonSubscriptions = subscriberData[@"non_subscriptions"];
    
    for (NSString *productID in nonSubscriptions) {
        NSArray *purchases = nonSubscriptions[productID];
        
        for (NSDictionary *purchase in purchases) {
            NSString *dateString = purchase[@"purchase_date"];
            
            while (parsedPurchases[dateString] != nil) {
                NSDate *date = [dateFormatter dateFromString:dateString];
                dateString = [dateFormatter stringFromDate:[date dateByAddingTimeInterval:10]];
            }
            [parsedPurchases setValue:productID forKey:dateString];
        }
    }
